...
@click.option('-h', '--horizon', type=int, help='Rollout horizon, when mode is record', default=500)
...
def main(env_name, rollout_path, mode, keyboard, horizon, seed, num_repeat, render, camera_name, output_dir, output_name, save_paths, compress_paths, plot_paths, env_args, noise_scale):
    ...
    # load paths
    # if keyboard:
    from vtils.keyboard import xbox_input as keyboard 
    ky = keyboard.XboxController()
    ...
    while count < num_repeat:
        ...
        if len(demo_paths) > 0:
            ...
            env.env.robot.robot_config['franka']['robot'].gain_scale = gain_scale
    ...
    print("Saving Paths!")
    if save_paths:
        file_name = os.path.join(output_dir, output_name + 'teleop_{}_paths.pickle'.format(time_stamp))
